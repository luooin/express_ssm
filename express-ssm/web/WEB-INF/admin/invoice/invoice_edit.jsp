        ...
        <div class="panel-body" style="padding-bottom:30px;">
            <form class="form-horizontal" method="post" id="commentForm" enctype="multipart/form-data"
                        ...
                        <select name='car' class='form-control'>
                            ...
                            <c:forEach varStatus="status" var="cars" items="${carsList}">
                                <option value="${cars.id}"
                                        <c:if test="${invoice.car == cars.id}">selected</c:if>>${cars.carNumber}</option>
                            </c:forEach>
                        </select>
                        <div class='tip1'></div>
                    </div>
                        ...
                        <select name='driver' class='form-control'>
                            <c:forEach varStatus="status" var="drivers" items="${driversList}">
                                <option value="${drivers.id}"
                                        <c:if test="${invoice.driver == drivers.id}">selected</c:if>>${drivers.username}</option>
                            </c:forEach>
                        </select>
                        <div class='tip1'></div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input type="text" placeholder="发货日期"
                               value='<fmt:formatDate value="${invoice.sendTime}" pattern="yyyy-MM-dd"/>'
                               class="form-control" name="sendTime" id="sendTime">
                        <div class='tip1'></div>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <select name="startSite" class="form-control">
                            ...
                            <c:forEach varStatus="status" var="sites" items="${sitesList}">
                                <option value="${sites.id}" <c:if test="${invoice.startSite == sites.id}">selected</c:if>>${sites.region}-${sites.siteName}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <select name="endSite" class="form-control">
                            <option>请选择</option>
                            <c:forEach varStatus="status" var="sites" items="${sitesList}">
                                <option value="${sites.id}" <c:if test="${invoice.endSite == sites.id}">selected</c:if>>${sites.region}-${sites.siteName}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <label><input type='radio' name='state' value='0' <c:if
                                test="${invoice.state == 0}"> checked</c:if>/> 派送中</label>
                        <label><input type='radio' name='state' value='1' <c:if
                                test="${invoice.state == 1}"> checked</c:if>/> 完成</label>
                        <div class='tip1'></div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="col-sm-offset-3 col-sm-8">
                        <button class="btn btn-sm btn-info" onclick="update('invoice')">提 交</button>
                    </div>
                </div>
            </form>
        </div>
